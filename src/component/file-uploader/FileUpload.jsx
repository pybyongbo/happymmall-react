...
class FileUpload extends React.Component {
    ...
    commonUpload() {
        ...
        xhr.onreadystatechange = () => {
            ...
            try {
                ...
                /*超时抛出不一样的错误，不在这里处理*/
                !scope.isTimeout && this.uploadError({
                    type: 'FINISHERROR',
                    message: e.message
                })
            }
    ...
    _multiIEForm() {
         ...
         * 所以当disabledIEChoose为true（或者func返回值为true）时，禁止IE上传。
         */
        const isDisabled = typeof this.disabledIEChoose === 'function' ? this.disabledIEChoose() : this.disabledIEChoose
        ...
        function _insertIEForm(formArr, i) {
            ...
            const input = <input type="file" name={`ajax_upload_hidden_input_${i}`} id={`ajax_upload_hidden_input_${i}`}
            ref={`ajax_upload_hidden_input_${i}`} onChange={(e) => {
                this.IEChooseFile(e)
            }} onClick={(e) => {
                this.IEBeforeChoose(e)
            }}
            ...
            formArr.push((
                ...
                encType="multipart/form-data" ref={`form_${i}`} onSubmit={(e) => {
                    this.IEUpload(e)
                }}
                style={{
